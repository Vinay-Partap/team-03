...
export default function Register() {
  ...
  const input = "w-full h-12 pl-11 pr-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all font-medium text-sm";
  return <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[480px] bg-[#0b1329]/50 backdrop-blur-xl border border-white/10 rounded-3xl p-7 sm:p-8 shadow-[0_0_50px_rgba(59,130,246,.08)]">
    <Toaster position="top-right" /><div className="mb-5 text-center"><h2 className="text-3xl font-extrabold text-white">Create your account</h2><p className="mt-1 text-sm text-slate-400">Join GovIntel Platform</p></div>
    <form className="space-y-3" onSubmit={submit}>
      ...
      <motion.button whileTap={{scale:.99}} type="submit" disabled={loading} className="mt-2 w-full h-12 flex items-center justify-center gap-2 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-500 disabled:opacity-50">{loading ? "Registering..." : <><UserCheck className="h-5 w-5"/>Create Account</>}</motion.button>
    </form><p className="mt-5 text-center text-sm text-slate-400">Already have an account? <Link to="/login" className="font-semibold text-blue-400">Login here <ArrowRight className="inline h-4 w-4"/></Link></p>
  </motion.div>;
}
