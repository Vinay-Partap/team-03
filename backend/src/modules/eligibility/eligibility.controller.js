...
const evaluateScheme = (profile, scheme) => {
  ...
  // 5. Category check (Caste/Reservation)
...
const checkEligibility = async (req, res) => {
  try {
    const profile = req.body; // attributes: age, gender, income, occupation, education, state, category, disability
    const schemes = await Scheme.find({ status: "approved" });
...
const checkMyEligibility = async (req, res) => {
  try {
    ...
    const schemes = await Scheme.find({ status: "approved" });
    const results = schemes.map((scheme) => evaluateScheme(req.user.profile, scheme));
