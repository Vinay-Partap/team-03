      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
          <div className="flex justify-between items-start">
            <div className="bg-emerald-50 text-emerald-600 h-10 w-10 rounded-xl flex items-center justify-center">
          ...
          <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-100 text-center text-xs font-semibold">
            <div>
              <p className="text-slate-400">Live</p>
              <p className="text-emerald-600 font-bold mt-0.5">{stats.policies.approved}</p>
            </div>
            <div>
              <p className="text-slate-400">Pending</p>
              <p className="text-orange-500 font-bold mt-0.5">{stats.policies.pending}</p>
            </div>
            <div>
              <p className="text-slate-400">Drafts</p>
              <p className="text-slate-500 font-bold mt-0.5">{stats.policies.draft}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
          <div className="flex justify-between items-start">
            <div className="bg-purple-50 text-purple-600 h-10 w-10 rounded-xl flex items-center justify-center">
            ...
            >
              <Plus className="h-3 w-3" /> New Scheme
            </Link>
          </div>
          <div>
          ...
          <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-100 text-center text-xs font-semibold">
            <div>
              <p className="text-slate-400">Live</p>
              <p className="text-purple-600 font-bold mt-0.5">{stats.schemes.approved}</p>
            </div>
            <div>
              <p className="text-slate-400">Pending</p>
              <p className="text-orange-500 font-bold mt-0.5">{stats.schemes.pending}</p>
            </div>
            <div>
              <p className="text-slate-400">Drafts</p>
              <p className="text-slate-500 font-bold mt-0.5">{stats.schemes.draft}</p>
            </div>
          </div>
        </div>

        {/* Workflow Approvals Queue */}
        <div className="bg-gradient-to-br from-slate-900 to-emerald-950 text-white rounded-3xl p-6 shadow-md flex flex-col justify-between">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
          ...
          <div className="space-y-3 pt-2">
            {stats.breakdowns.policyCategories.length === 0 ? (
              <p className="text-slate-400 text-xs text-center py-6">No data registered.</p>
            ) : (
              stats.breakdowns.policyCategories.map((c) => (
                <div key={c._id} className="flex justify-between items-center text-xs">
                  ...
                  <span className="bg-slate-50 border border-slate-100 font-bold px-3 py-1 rounded-lg text-slate-700">
                    {c.count} policies
                  </span>
                </div>
              ))
