    ...
    <div className="space-y-8 animate-gradient">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2"><div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm"><h3 className="font-bold text-slate-800">Application Status</h3><div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600">{Object.entries(stats.applicationStatus || {}).length ? Object.entries(stats.applicationStatus).map(([status,count])=><div key={status} className="rounded-lg bg-slate-50 p-2 capitalize"><b>{count}</b> {status.replace("_"," ")}</div>) : <p>No applications yet.</p>}</div></div><div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm"><h3 className="font-bold text-slate-800">Recent Searches</h3><div className="mt-3 space-y-2">{stats.recentSearches?.length ? stats.recentSearches.map(q=><Link key={q} to={`/policies?search=${encodeURIComponent(q)}`} className="block text-sm text-blue-600">{q}</Link>) : <p className="text-xs text-slate-500">No searches yet.</p>}</div></div></div>
